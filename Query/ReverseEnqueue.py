...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("./ReverseEnqueue.py output_folder network_specification_file S_gene_name P_gene_name [command]")
        exit(1)
    if len(sys.argv) == 6:
        command = sys.argv[5]
    else:
    ...
    query_script = str(os.path.dirname(os.path.realpath(__file__))) + "/ReverseQuery.py"
